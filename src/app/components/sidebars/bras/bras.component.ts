...
export class SidebarBrasComponent extends AbstractComponent implements OnInit, AfterViewInit {
  ...
  async ngAfterViewInit() {
    ...
    ['bas', 'haut'].forEach((idBras: BRAS) => {
      ...
      this.bras[idBras].add(new Konva.Line({
        name       : 'line',
        points     : [0, 0, 0, 0],
        stroke     : 'black',
        lineCap    : 'round',
    ...
    this.brasSym.add(new Konva.Rect({
      name       : 'echantillon',
      x          : 0,
      y          : 0,
      width      : 15 * RATIO,
  ...
  isStateDisabled(bras: BRAS, state: string) {
    return this.current?.[bras].state &&
      !this.config[bras === 'bas' ? 'transitionsBas' : 'transitionsHaut']
        .some((transition) => {
  ...
  private async setCursor() {
    ...
    this.brasService.calculerAngles(robot, this.selectedBras, { ...pt, a: this.angleVentouse })
      .subscribe((result) => {
        if (result) {
