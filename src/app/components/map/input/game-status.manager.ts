...
export class GameStatusManager {
  ...
  private createBouee(x: number, y: number, couleur: ECouleurBouee, text?: string) {
    ...
    group.add(
      new Konva.Circle({
        x          : x * TABLE.imageRatio,
        y          : y * TABLE.imageRatio,
        radius     : this.DIAMETRE_GOBELET * TABLE.imageRatio / 2,
    ...
    if (text) {
      group.add(
        new Konva.Text({
          text,
          x            : (x - this.DIAMETRE_GOBELET / 2) * TABLE.imageRatio,
          y            : (y - this.DIAMETRE_GOBELET / 2) * TABLE.imageRatio,
          width        : this.DIAMETRE_GOBELET * TABLE.imageRatio,
          height       : this.DIAMETRE_GOBELET * TABLE.imageRatio,
          fontSize     : 16,
          fontStyle    : 'bold',
          align        : 'center',
