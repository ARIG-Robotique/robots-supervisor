...
export class MapInputComponent extends AbstractComponent implements OnChanges, OnDestroy, AfterViewInit {
                ...
                switch (data.targetMvt.type) {
                    case 'PATH':
                        data.targetMvt.path.forEach((point) => {
                            points.push(point.x * TABLE.imageRatio, (TABLE.height - point.y) * TABLE.imageRatio);
                        });
    ...
    mousedown(e) {
        ...
        this.state.startX = $event.clientX - this.mapContainer.nativeElement.offsetLeft;
        this.state.startY = $event.clientY - this.mapContainer.nativeElement.offsetTop;
    ...
    mousemove(e) {
        ...
        this.state.endX = $event.clientX - this.mapContainer.nativeElement.offsetLeft;
        this.state.endY = $event.clientY - this.mapContainer.nativeElement.offsetTop;
    ...
    moveCrosshair() {
        if (this.crosshair) {
            ...
            let blocked = false;
            if (this.mask) {
                const pixel = this.mask.getImageData(
                    this.state.position.x * TABLE.imageRatio,
                    this.state.position.y * TABLE.imageRatio,
                    1,
                    1,
                ).data;
                blocked = pixel[0] < 127;
            }
            for (const child of this.crosshair.children) {
